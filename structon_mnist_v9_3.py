...
class StateExtractor:
    ...
    def extract(self, image: np.ndarray) -> np.ndarray:
        ...
        # 3.3 上下左右差异
        features.append(np.mean(binary[:h//2, :]) - np.mean(binary[h//2:, :]))  # 上-下
        features.append(np.mean(binary[:, :w//2]) - np.mean(binary[:, w//2:]))  # 左-右
        
        # 3.4 中心 vs 边缘 (区分 0 vs 8)
        center = binary[h//4:3*h//4, w//4:3*w//4]
        features.append(np.mean(center))  # 中心密度
        
        # 中心的中心（最核心区域）- 0 应该是空的，8 应该有内容
        core = binary[h//3:2*h//3, w//3:2*w//3]
        features.append(np.mean(core))
        
        # 3.5 水平切片 (区分 1 vs 7, 检测横线)
        features.append(np.mean(binary[2:5, :]))         # 顶部
        features.append(np.mean(binary[h//2-2:h//2+2, :]))  # 中部
        features.append(np.mean(binary[-5:-2, :]))       # 底部
        
        # 3.6 垂直切片 (区分有无竖线)
        features.append(np.mean(binary[:, w//2-2:w//2+2]))  # 中央竖线
        features.append(np.mean(binary[:, 2:5]))         # 左边
        features.append(np.mean(binary[:, -5:-2]))       # 右边
        
        # 3.7 对角线特征 (区分 1 vs 7)
        diag1 = np.mean([binary[i, i] for i in range(min(h, w))])
        diag2 = np.mean([binary[i, w-1-i] for i in range(min(h, w))])
        features.append(diag1)  # 主对角线
        features.append(diag2)  # 副对角线
        features.append(diag1 - diag2)  # 对角线差异
        
        # 3.8 开口检测 (通过边缘密度)
        features.append(np.mean(binary[0, :]))   # 顶边
        features.append(np.mean(binary[-1, :]))  # 底边
...
# =============================================================================
class LRM:
    def __init__(self, state_dim=55, n_actions=4, capacity=200):
