...
class Structon:
    ...
    def learn(self, pattern: np.ndarray, action: Any, reward: float) -> Tuple[str, 'Structon']:
        ...
        if self.frozen:
            if self.children:
                ...
                best_score = -1.0
                for child in self.children:
                    _, score, _ = child.query(pattern)
                    if score > best_score:
                        best_score = score
                        best_child = child
                if best_child:
                    return best_child.learn(pattern, action, reward)
            return "frozen_no_child", self
    ...
    def promote(self) -> 'Structon':
        ...
        1. 冻结自己
        2. 复制自己作为 sibling（记忆继承但活跃）
        3. 创建 wrapper
        4. wrapper.children = [self, sibling]
        5. 返回 wrapper
        """
        self.promotions += 1
        ...
        # 2. 创建 sibling（复制记忆，但活跃）
        sibling = Structon(
            capacity=self.capacity,
            similarity_threshold=self.similarity_threshold
        )
        # 继承记忆（深拷贝）
        sibling.lrm.entries = copy.deepcopy(self.lrm.entries)
        sibling.lrm.frozen = False
        # 重置统计但保留记忆
        for entry in sibling.lrm.entries:
            entry.access_count = 0
            entry.success_count = 0
            entry.strength = 1.0  # 重置强度
