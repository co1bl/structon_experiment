...
class StructonVisionSystem:
    """
    Structon 视觉系统 v8.5 - 通过 LRM 联想进入
    
    核心设计：
    - 全局 LRM 存储所有样本的记忆
    - 每条记忆标记属于哪个 Structon
    - 查询时找最相似的记忆 → 从那个 Structon 开始
    """
    ...
    def __init__(
        ...
        # 全局 LRM - 用于联想入口
        # 存储 (key, owner_label) 对
        self.global_keys: List[np.ndarray] = []
        self.global_owners: List[str] = []  # 每条记忆属于哪个 Structon
        self.global_capacity = 500  # 全局记忆容量
        
        # 投影矩阵（与 Structon 共享维度）
        self.projection = np.random.randn(state_dim, key_dim).astype(np.float32)
        self.projection /= np.linalg.norm(self.projection, axis=0, keepdims=True)
    ...
    def _find_entry(self, state: np.ndarray) -> Optional[Structon]:
        """
        通过全局 LRM 找入口 Structon
        
        联想：找最相似的记忆，返回它属于的 Structon
        """
        if len(self.global_keys) == 0:
            # 没有记忆，随机选择
            if self.structons:
                return np.random.choice(list(self.structons.values()))
            return None
        
        key = self._compute_key(state)
        key_matrix = np.array(self.global_keys)
        scores = key_matrix @ key
        
        # 找最相似的记忆
        best_idx = int(np.argmax(scores))
        owner_label = self.global_owners[best_idx]
        
        if owner_label in self.structons:
            return self.structons[owner_label]
        
        # 找不到，随机选择
        return np.random.choice(list(self.structons.values()))
    
    def _remember_global(self, state: np.ndarray, label: str):
        """
        添加到全局记忆
        """
        key = self._compute_key(state)
        ...
        # 容量管理
        if len(self.global_keys) >= self.global_capacity:
            # 删除最旧的
            self.global_keys.pop(0)
            self.global_owners.pop(0)
        
        self.global_keys.append(key.copy())
        self.global_owners.append(label)
    ...
    def train(self, image: np.ndarray, label: str):
        ...
        # 1. 添加到全局记忆
        self._remember_global(state, label)
        ...
        path = []
        if entry:
            self._execute_and_record(state, path, entry)
    ...
    def print_stats(self):
        """打印统计"""
        print("\n" + "=" * 60)
        print("Structon Vision v8.5 - LRM 联想入口")
        print("=" * 60)
        print(f"Structon 数量: {len(self.structons)}")
        print(f"全局记忆: {len(self.global_keys)}/{self.global_capacity}")
        total_mem = sum(s.lrm.size for s in self.structons.values())
        print(f"Structon 总记忆: {total_mem}")
        
        # 统计全局记忆分布
        label_counts = {}
        for owner in self.global_owners:
            label_counts[owner] = label_counts.get(owner, 0) + 1
        print(f"\n全局记忆分布:")
