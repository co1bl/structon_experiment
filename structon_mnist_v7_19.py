...
class Structon:
    ...
    def __init__(self, capacity: int = 10, similarity_threshold: float = 0.85,
                 ...
                 projector: Optional[np.ndarray] = None, n_siblings: int = 2):
        ...
        self.n_siblings = n_siblings  # promote 时创建几个 sibling
    ...
    def query(self, pattern: np.ndarray) -> Tuple[Optional[str], float, Optional[int], 'Structon']:
        ...
        if self.children:
            # 收集所有 children 的投票
            votes = {}  # action -> (total_weight, best_sim, best_idx, best_structon)
            
            for child in self.children:
                action, child_sim, child_idx, responding = child.query(pattern)
                if action is not None and child_sim > 0:
                    # 用相似度作为投票权重
                    weight = child_sim
                    ...
                    votes[action]['total_weight'] += weight
                    votes[action]['count'] += 1
                    
                    # 记录最佳匹配（用于返回）
                    if child_sim > votes[action]['best_sim']:
                        votes[action]['best_sim'] = child_sim
                        votes[action]['best_idx'] = child_idx
                        votes[action]['best_structon'] = responding
            
            # 选择总权重最高的 action
            if votes:
                best_action = max(votes.keys(), key=lambda a: votes[a]['total_weight'])
                best_vote = votes[best_action]
                return (best_action, best_vote['best_sim'], 
                        best_vote['best_idx'], best_vote['best_structon'])
    ...
    def _add_new_memory(self, pattern: np.ndarray, action: str) -> Tuple[str, 'Structon']:
        ...
        if self.children:
            # 我是 wrapper → 找最合适的 active sibling
            # 选择：1) 有空间的 2) 投影后最相似的
            best_child_idx = None
            best_sim = -1.0
            ...
            for i, child in enumerate(self.children):
                if child.frozen:
                    continue
                # 优先选择有空间的
                if child.lrm.size() < child.capacity:
                    # 在有空间的 sibling 中选投影后最相似的
                    child_local = child._local(pattern)
                    _, sim = child.lrm.query(child_local)
                    if sim > best_sim:
                        best_sim = sim
                        best_child_idx = i
            
            if best_child_idx is not None:
                status, returned = self.children[best_child_idx]._add_new_memory(pattern, action)
                if "promoted" in status:
                    self.children[best_child_idx] = returned
                return status, self
            
            # 所有 sibling 都满了 → 找第一个非 frozen 的 promote
            for i, child in enumerate(self.children):
                if not child.frozen:
    ...
    def promote(self) -> 'Structon':
        """
        晋升：冻结 → 创建多个 sibling（不同视角） → 包裹
        
        frozen: 保留所有记忆（保护旧知识）
        siblings: 每个有不同的随机投影（多视角集成）
        wrapper: 继承原投影，负责路由
        """
        # 1. 冻结自己
        self.freeze()
        
        # 2. 创建多个 sibling，每个有不同的随机投影！
        siblings = []
        for i in range(self.n_siblings):
            sibling = Structon(
                ...
                projector=None,  # 新的随机投影！不同视角
                n_siblings=self.n_siblings
            )
            siblings.append(sibling)
        
        # 3. 创建 wrapper（继承投影，保持路由一致性）
        wrapper = Structon(
            capacity=self.capacity,
            similarity_threshold=self.similarity_threshold,
            dopamine_threshold=self.dopamine_threshold,
        ...
        # 4. 建立关系：[frozen, sibling1, sibling2, ...]
        wrapper.children = [self] + siblings
...
def run_experiment(n_per_class=20, n_test=500, capacity=10, threshold=0.85,
                   dopamine_threshold=0.1, key_dim=8, n_siblings=3, verbose=True):
    """
    按类别训练实验
    ...
    """
    print("=" * 70)
    print("Structon Vision v7.19 - 投票集成 + 多巴胺驱动分形学习")
    print("=" * 70)
    print(f"\n参数:")
    ...
    print(f"  n_siblings={n_siblings} (每次 promote 创建 {n_siblings} 个不同视角)")
    print(f"  每类训练: {n_per_class}, 测试: {n_test}")
    print("\n机制:")
    print("  1. 投票集成：所有 children 投票，加权聚合")
    print("  2. 多视角：每个 sibling 有不同的随机投影")
    print("  3. 多巴胺平滑：惊讶驱动学习")
    print("  4. 分形生长：多分支树结构")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dopamine', type=float, default=0.1, help='多巴胺阈值（更低=更多更新）')
    parser.add_argument('--key-dim', type=int, default=8, help='投影后的维度')
    parser.add_argument('--siblings', type=int, default=3, help='每次promote创建几个sibling')
