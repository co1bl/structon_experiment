...
class Structon:
    ...
    def __init__(self, capacity: int = 10, similarity_threshold: float = 0.85,
                 dopamine_threshold: float = 0.3, full_dim: int = 25, concern_dim: int = 8):
        ...
        # Local Concern: 只关注部分特征
        self.full_dim = full_dim
        self.concern_dim = min(concern_dim, full_dim)
        self.concern_indices = np.random.choice(full_dim, self.concern_dim, replace=False)
    ...
    def _local(self, pattern: np.ndarray) -> np.ndarray:
        """提取局部关注的特征"""
        return pattern[self.concern_indices]
    ...
    def promote(self) -> 'Structon':
        ...
        frozen: 保留所有记忆（保护旧知识）
        sibling: 空的！从零开始学新东西
        关键：sibling 和 wrapper 继承相同的 concern_indices
        """
        # 1. 冻结自己
        self.freeze()
        
        # 2. 创建空的 sibling（继承相同的 concern！）
        sibling = Structon(
            capacity=self.capacity,
            similarity_threshold=self.similarity_threshold,
            dopamine_threshold=self.dopamine_threshold,
            full_dim=self.full_dim,
            concern_dim=self.concern_dim
        )
        sibling.concern_indices = self.concern_indices.copy()  # 继承 concern
        
        # 3. 创建 wrapper（也继承相同的 concern！）
        wrapper = Structon(
            capacity=self.capacity,
            similarity_threshold=self.similarity_threshold,
            dopamine_threshold=self.dopamine_threshold,
        ...
        wrapper.concern_indices = self.concern_indices.copy()  # 继承 concern
    ...
    def print_tree(self, indent: int = 0):
        ...
        concern_str = ",".join(map(str, sorted(self.concern_indices)))
        
        print(f"{prefix}{icon} {self.id} (mem:{self.lrm.size()}/{self.capacity}, "
              f"concern:[{concern_str}])")
...
def run_experiment(n_per_class=20, n_test=500, capacity=10, threshold=0.85,
                   dopamine_threshold=0.3, concern_dim=8, verbose=True):
    """
    按类别训练实验
    ...
    """
    print("=" * 70)
    print("Structon Vision v7.15 - Local Concern + 多巴胺驱动分形学习")
    print("=" * 70)
    print(f"\n参数:")
    print(f"  capacity={capacity}, threshold={threshold}")
    print(f"  dopamine_threshold={dopamine_threshold}")
    print(f"  concern_dim={concern_dim} (每个Structon只关注{concern_dim}维特征)")
    print(f"  每类训练: {n_per_class}, 测试: {n_test}")
    print("\n机制:")
    print("  1. Local Concern：每个Structon只关注部分特征")
    print("  2. 多巴胺门控：惊讶才学习")
    print("  3. 按类别训练：先学完一类再学下一类")
    print("  4. 分形生长：promote时新节点有新视角")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--concern', type=int, default=8, help='每个Structon关注的特征维度')
