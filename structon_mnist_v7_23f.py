...
class WrapperStructon:
    ...
    def learn(self, state: np.ndarray, true_label: str) -> Tuple[str, float]:
        ...
        核心逻辑：
        1. 左边（冻结子树）尝试处理
        2. 如果左边惊讶高（处理不了）→ 右边 Atomic 学习处理
        3. Wrapper 学习：哪种输入走哪边
        
        惊讶反向传播：下层惊讶影响上层的路由学习
        """
        self.total_learns += 1
        ...
        if isinstance(self.children[0], WrapperStructon):
            ...
            left_surprise = 0.0 if left_correct else 1.0
        ...
        # 3. 关键决策：左边能处理吗？
        # 判断标准：左边正确 且 惊讶低
        left_can_handle = left_correct and (left_surprise < 0.5)
        ...
        if left_can_handle:
            ...
            if not self.children[1].frozen:
                self.children[1].learn(state, true_label)
            # 给右边一个弱正向信号（鼓励它学习这类输入）
            self.lrm.remember(state, action=1, target_q=0.3)
            self.lrm.remember(state, action=0, target_q=-left_surprise)
        ...
        # 如果走左边，传递左边惊讶；否则用右边结果
        if left_can_handle:
            total_surprise = left_surprise * 0.5  # 左边能处理，低惊讶
        else:
