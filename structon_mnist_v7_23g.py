...
class WrapperStructon:
    ...
    def learn(self, state: np.ndarray, true_label: str) -> Tuple[str, float]:
        ...
        核心逻辑：
        1. 左边（冻结子树）尝试处理
        2. 如果左边处理不了 → 右边 Atomic 学习处理
        3. Wrapper 学习路由：哪种输入走哪边
            ...
            if wrapper_ready:
                ...
                if isinstance(self.root, WrapperStructon):
                    print(f"  ★ 熟练! 连续正确 {self.mastery_threshold} 次，"
                          f"树惊讶低(avg:{avg_surprise:.2f})，等待下一个样本触发 promote")
                else:
                    print(f"  ★ 熟练! 连续正确 {self.mastery_threshold} 次，等待下一个样本触发 promote")
            else:
                # Wrapper 还没学好，继续学习
                self.consecutive_correct = 0  # 重置，但不 promote
                print(f"  ◆ Atomic 熟练但 Wrapper 惊讶高(avg:{avg_surprise:.2f}, max:{max_surprise:.2f})，继续学习路由")
...
def run_experiment(
    ...
    for digit in range(10):
        class_indices[digit] = np.where(train_labels == digit)[0][:n_per_class]
        np.random.shuffle(class_indices[digit])  # 打乱顺序
    ...
    # 混合训练：轮流从每个类别取样本
    # 每轮从 0-9 各取一个，重复 n_per_class 轮
    print(f"\n开始混合训练，每类 {n_per_class} 样本，共 {n_per_class * 10} 样本")
    
    stats_window = []  # 用于计算滑动准确率
    
    for round_idx in range(n_per_class):
        # 随机打乱这一轮的类别顺序
        digits_this_round = list(range(10))
        np.random.shuffle(digits_this_round)
        
        for digit in digits_this_round:
            if class_pointers[digit] >= len(class_indices[digit]):
                continue  # 这个类别用完了
            
            idx = class_indices[digit][class_pointers[digit]]
            class_pointers[digit] += 1
        ...
        # 每 20 轮打印一次进度
        if (round_idx + 1) % 20 == 0:
            recent_acc = sum(stats_window) / len(stats_window) * 100 if stats_window else 0
            avg_surprise = system.get_recent_surprise(50)
            print(f"  轮次 {round_idx+1}/{n_per_class}: "
                  f"样本 {total_samples_used}, "
                  f"滑动准确率 {recent_acc:.1f}%, "
                  f"惊讶 {avg_surprise:.2f}, "
