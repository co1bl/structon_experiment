...
class WrapperStructon:
    ...
    def learn(self, state: np.ndarray, true_label: str) -> str:
        ...
        # 执行选择的方向
        result, _ = self.children[action].execute(state)
        
        if result == true_label:
            # 选对了！强化这个动作
            self.lrm.remember(state, action=action, target_q=1.0)
            status = f'correct_action{action}'
        else:
            # 选错了！弱化这个动作，强化另一个
            self.lrm.remember(state, action=action, target_q=-0.5)
            other_action = 1 - action
            self.lrm.remember(state, action=other_action, target_q=0.8)
            status = f'wrong_action{action}'
        
        # 右边的 Atomic 也要学习（如果未冻结）
...
class StructonVisionSystem:
    ...
    def train_one(self, image: np.ndarray, true_label: str) -> Tuple[bool, float]:
        ...
        # 如果等待 promote（上一轮熟练了），用新样本的 label 创建新 Atomic
        ...
        # 检查熟练度：连续正确 >= threshold
        if self.consecutive_correct >= self.mastery_threshold:
            # 熟练了！标记等待 promote
            # 下一个样本来的时候再真正 promote（用新样本的 label）
...
def run_experiment(
    ...
    """运行实验"""
    print("=" * 70)
    print("Structon Vision v7.23 - 多巴胺驱动的二分分形架构")
    print("=" * 70)
    print(f"\n参数:")
    print(f"  capacity={capacity}, key_dim={key_dim}")
    print(f"  mastery_threshold={mastery_threshold} (连续正确次数)")
    print(f"  每类最多训练: {n_per_class}, 测试: {n_test}")
    
    print("\n核心设计:")
    print("  1. Atomic: [是X, 不是X]，有 LRM")
    print("  2. Wrapper: [走左, 走右]，有 LRM")
    print("  3. 多巴胺驱动: 连续正确 → 熟练 → promote")
    print("  4. 结构不知道'类别'，由训练数据决定")
