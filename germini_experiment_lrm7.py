...
class StateExtractor:
    ...
    def extract(self, image: np.ndarray) -> np.ndarray:
        ...
        features.append((np.mean(grid[:3, :]) - np.mean(grid[4:, :])) * 2) 
        features.append((np.mean(grid[:, :3]) - np.mean(grid[:, 4:])) * 2)
...
class DynamicVisionSystem:
    ...
    def navigate(self, image, target_label=None, train=False):
        ...
        while momentum > 0.1:
            ...
            conf = q_vec[action]
            path_history.append(current.label)
            
            # --- 检测震荡与循环 ---
            if current.label in visited:
                momentum -= 0.2 # 发现循环，大幅扣除动能，强制逃逸或终止
            visited.append(current.label)
            if len(visited) > 4: visited.pop(0) # 保持短时记忆

            if action == 0: # 尝试认领
                if train and target_label:
                    if current.label == target_label:
                        current.lrm.update(state, 0, 5.0) # [对] 极深吸引子
                        return current.label, path_history
                    else:
                        current.lrm.update(state, 0, -4.0) # [错] 极高排斥子
                        momentum -= 0.4
                elif conf > 0.8: # 推理时：只要够稳定就满足
                    return current.label, path_history
                else:
                    momentum -= 0.15
...
if __name__ == "__main__":
    ...
    for ep in range(10):
        ...
        print(f"Epoch {ep+1:2d} | 满足准确率: {correct/200*100:4.1f}%")

    res, path = sys.navigate(test_img[0])
    print(f"\n示例路径 (检测震荡后): {' -> '.join(path)}")
