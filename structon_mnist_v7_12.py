...
class Structon:
    ...
    def learn(self, pattern: np.ndarray, true_action: str) -> Tuple[str, 'Structon']:
        ...
        if predicted_action == true_action:
            # 预测对了 → 不惊讶 → 强化 + 习惯化
            self.skipped += 1
            dopamine = 0.1  # 小正反馈，强化
            responding.lrm.update_with_dopamine(idx, dopamine)
            return "correct_reinforced", self
        else:
            # 预测错了 → 惊讶！→ 需要修正
            self.learns += 1
            dopamine = -1.0  # 负反馈，弱化错误记忆
            responding.lrm.update_with_dopamine(idx, dopamine)
            
            # 同时存储正确的记忆
            if self.children:
                # 我是 wrapper → 交给 sibling
                for i, child in enumerate(self.children):
                    if not child.frozen:
                        status, returned = child.learn(pattern, true_action)
                        if "promoted" in status:
                            self.children[i] = returned
                        return "wrong_corrected", self
                return "wrong_no_sibling", self
            else:
                # 我是叶子 → 直接添加正确记忆
                if not self.frozen:
                    if not self.lrm.is_full():
                        self.lrm.add(pattern, true_action, expectation=0.0)
                        return "wrong_added_correct", self
                    else:
                        new_wrapper = self.promote()
                        for child in new_wrapper.children:
                            if not child.frozen:
                                child.lrm.add(pattern, true_action, expectation=0.0)
                                break
                        return "wrong_promoted_added", new_wrapper
                return "wrong_frozen", self
