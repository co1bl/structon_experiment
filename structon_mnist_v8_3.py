...
class StructonVisionSystem:
    ...
    def predict(self, image: np.ndarray, entry: Structon = None) -> Tuple[str, float]:
        """预测 - 可以指定入口"""
        ...
        # 如果没指定入口，随机选择一个
        if entry is None:
            entry = np.random.choice(list(self.structons.values()))
    ...
    def predict_best(self, image: np.ndarray) -> Tuple[str, float]:
        """
        预测 - 尝试所有入口，返回置信度最高的
        """
        state = self.extractor.extract(image)
        ...
        for structon in self.structons.values():
            label, conf = structon.execute(state)
            if conf > best_conf:
                best_conf = conf
                best_label = label
...
def run_experiment(
    ...
    for idx in test_indices:
        predicted, _ = system.predict_best(test_images[idx])
