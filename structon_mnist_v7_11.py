...
class Structon:
    ...
    def query(self, pattern: np.ndarray) -> Tuple[Optional[str], float, Optional[int], 'Structon']:
        ...
        # 本层共振
        idx, sim = self.lrm.query(pattern)
        if idx is not None:
            self.local_hits += 1
    ...
    def learn(self, pattern: np.ndarray, true_action: str, actual_feeling: float) -> Tuple[str, 'Structon']:
        """
        多巴胺驱动学习
        
        actual_feeling: +1 (正确) 或 -1 (错误)
        
        学习逻辑：
        1. 查询 → 预测
        2. 计算多巴胺 = actual - expected
        3. 门控：|δ| < threshold → 跳过
        4. wrapper 学路由，sibling 学具体知识
        """
        # 1. 查询
        ...
        # 2. 计算多巴胺
        if idx is not None:
            expected = responding.lrm.get_expectation(idx)
        else:
            expected = 0.0
        
        dopamine = actual_feeling - expected
        
        # 3. 门控：只有惊讶才学习
        if abs(dopamine) < self.dopamine_threshold:
            self.skipped += 1
            return "skipped_not_surprised", self
        ...
        if self.children:
            ...
            # 检查 frozen children 是否认识
            frozen_knows = False
            frozen_correct = False
            for child in self.children:
                if child.frozen:
                    child_idx, child_sim = child.lrm.query(pattern)
                    if child_idx is not None:
                        frozen_knows = True
                        if child.lrm.get_action(child_idx) == true_action:
                            frozen_correct = True
                        break
            
            if frozen_knows and frozen_correct:
                # frozen 认识且正确 → 不需要学
                return "frozen_correct", self
            
            if frozen_knows and not frozen_correct:
                # frozen 认识但错了 → sibling 需要学正确的
                # wrapper 也学：这个 pattern 不该去 frozen
                if not self.lrm.is_full() and not self.frozen:
                    self.lrm.add(pattern, true_action, expectation=0.0)
            
            # 交给 active sibling 学习
            for i, child in enumerate(self.children):
                if not child.frozen:
                    status, returned = child.learn(pattern, true_action, actual_feeling)
        ...
        if idx is not None and responding == self:
            # 本层有匹配 → 更新
            self.lrm.update_with_dopamine(idx, dopamine)
            return "updated", self
        ...
        if not self.lrm.is_full():
            ...
            for child in new_wrapper.children:
                if not child.frozen:
                    child.lrm.add(pattern, true_action, expectation=0.0)
                    break
    ...
    def print_tree(self, indent: int = 0):
        ...
        print(f"{prefix}{icon} {self.id} (mem:{self.lrm.size()}/{self.capacity}, "
              f"learns:{self.learns}, skipped:{self.skipped})")
...
class StructonVisionSystem:
    ...
    def train_one(self, image: np.ndarray, true_action: str) -> Tuple[str, bool]:
        ...
        # 先预测
        predicted, sim, _, _ = self.root.query(state)
        ...
        if predicted == true_action:
            ...
            actual_feeling = 0.0  # 没预测，中性
            correct = False
        else:
            actual_feeling = -1.0  # 预测错了
            correct = False
        
        # 学习
        status, returned = self.root.learn(state, true_action, actual_feeling)
...
def run_experiment(n_per_class=20, n_test=500, capacity=10, threshold=0.85,
                   dopamine_threshold=0.3, verbose=True):
    """
    按类别训练实验
    ...
    """
    print("=" * 70)
    print("Structon Vision v7.11 - 完整多巴胺驱动分形学习")
    print("=" * 70)
    print(f"\n参数:")
    ...
    print("\n机制:")
    print("  1. 多巴胺门控：|δ| > threshold 才学习")
    print("  2. 预期习惯化：重复 → 不惊讶 → 不学")
    print("  3. 按类别训练：先学完一类再学下一类")
    print("  4. sibling 继承记忆")
